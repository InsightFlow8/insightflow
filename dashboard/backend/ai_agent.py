...
def create_hybrid_agent(session_id: str = None, user_id: str = None):
    """Create a hybrid agent with conversation memory and async support"""
    
    try:
        logger.info(f"Creating hybrid agent - Session ID: {session_id}, User ID: {user_id}, Async: Enabled")
        ...
        # Create tools dynamically with user_id context and async support
        tools = create_enhanced_tools(vectorstore, user_id)
        
        logger.info(f"Created {len(tools)} tools with async support for optimal performance")
...
PERFORMANCE NOTE: This agent is running in async mode for optimal performance with concurrent processing of database queries, S3 operations, and ML model predictions."""
            }
        )
        
        logger.info("Agent created successfully with async support for optimal performance")
        return agent
...
async def create_hybrid_agent_async(session_id: str = None, user_id: str = None):
    """Create a hybrid agent with async support for optimal performance"""
    
    try:
        logger.info(f"Creating async hybrid agent - Session ID: {session_id}, User ID: {user_id}")
        ...
        logger.info(f"Created {len(tools)} async tools for optimal performance")
...
PERFORMANCE NOTE: This agent is running in async mode for optimal performance with concurrent processing of database queries, S3 operations, and ML model predictions."""
        ...
        logger.info("Async agent created successfully for optimal performance")
        return agent
        
    except Exception as e:
        logger.error(f"Error creating async hybrid agent: {e}")
        raise
...
async def process_chat_query_async(query: str, session_id: str = None, user_id: str = None):
    """Process a chat query using the async hybrid agent for optimal performance"""
    ...
    try:
        logger.info(f"Processing async query: '{query}' with session_id: {session_id}, user_id: {user_id}")
        
        # Pre-check for greetings and recommendation requests to provide direct responses
        query_lower = query.lower().strip()
        
        if is_greeting(query_lower):
            logger.info("Handling greeting query (async)")
            if user_id:
                response = f"Hello! I'm your product assistant. I can help you with product information and personalized recommendations. Your user ID is {user_id}, so I can provide personalized recommendations for you. Ask me 'What should I buy?' for personalized recommendations or ask about specific products and categories."
            else:
                response = "Hello! I'm your product assistant. I can help you with product information and personalized recommendations. Please enter your user ID above for personalized recommendations, or ask me about specific products and categories."
        ...
        # Handle recommendation requests without user ID directly
        if any(word in query_lower for word in ["recommend", "suggest", "buy", "should"]) and not user_id and not any(word in query_lower for word in ["show", "find", "search", "chips", "bread", "fruits", "vegetables", "dairy"]):
            logger.info("Handling recommendation request without user ID (async)")
            response = "I'd be happy to provide personalized recommendations! Please enter your user ID in the field above and try again. For example, enter '123' in the User ID field, then ask 'What should I buy?'"
        ...
        # For all other queries, use the async agent
        logger.info("Creating async hybrid agent for query processing")
        agent = await create_hybrid_agent_async(session_id, user_id)
        
        # Use ainvoke for async execution as recommended by LangChain
        logger.info("Running async agent with query")
        response = await agent.ainvoke({"input": query})
        logger.info(f"Async agent response received: {response[:100] if isinstance(response, str) else str(response)[:100]}...")
        ...
        return {
            "answer": answer,
            "type": "async_hybrid_agent_response",
            "session_id": session_id
        }
